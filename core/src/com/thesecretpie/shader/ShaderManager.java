...
public class ShaderManager {
	...
	public static final String SHADER_CLASSPATH = " ";
	...
	public ShaderManager(String shaderDir, AssetManager am, boolean addProcessors) {
		...
		setAssetManager(am);
		/*
		add("empty", Gdx.files.internal("empty.vert"), Gdx.files.internal("empty.frag"));
		add("default", Gdx.files.internal("default.vert"), Gdx.files.internal("default.frag"));
		if (addProcessors && (Gdx.app.getType() == ApplicationType.Desktop
				|| Gdx.app.getType() == ApplicationType.Applet
				|| Gdx.app.getType() == ApplicationType.WebGL)) {
			add("processor", Gdx.files.internal("processor.vert"), Gdx.files.internal("processor.frag"));
			add("processor_blur", Gdx.files.internal("processor.vert"), Gdx.files.internal("processor_blur.frag"));
			add("copy", Gdx.files.internal("processor.vert"), Gdx.files.internal("copy.frag"));
			add("processor_draw", Gdx.files.internal("processor.vert"), Gdx.files.internal("processor_draw.frag"));
			add("processor_draw_additive", Gdx.files.internal("processor.vert"), Gdx.files.internal("processor_draw_additive.frag"));
			add("processor_fill", Gdx.files.internal("processor.vert"), Gdx.files.internal("processor_fill.frag"));
		}
		*/
	}
	...
	public Mesh createScreenQuad() {
		Mesh sq = new Mesh(true, 4, 6, new VertexAttribute(Usage.Position, 3, "a_position"), new VertexAttribute(Usage.ColorUnpacked, 4, "a_color"), new VertexAttribute(Usage.TextureCoordinates, 2, "a_texCoords"));
		...
		sq.setVertices(new float[] {vec0.x, vec0.y, 0, 1, 1, 1, 1, 0, 1, vec1.x, vec0.y, 0, 1, 1, 1, 1, 1, 1, vec1.x, vec1.y, 0, 1, 1, 1, 1, 1, 0, vec0.x, vec1.y, 0, 1, 1, 1, 1, 0, 0});
		sq.setIndices(new short[] {0, 1, 2, 2, 3, 0});
		return sq;
	}
	...
	public void renderFB(FrameBuffer fb, Mesh out, String textureUniformName, int textureId) {
		if (fb != null) {
			...
			getCurrent().setUniformMatrix("u_projTrans", screenCamera.combined);
	 ...
	 */
	public ShaderProgram setVertexAttribute(java.lang.String name, int size, int type, boolean normalize, int stride, int offset) {
		if (currentShader != null) {
